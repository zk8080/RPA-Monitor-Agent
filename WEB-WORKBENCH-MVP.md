...
- **用户：** 开发人员（本机维护影刀流程）
- **主路径：** 总览 → 应用列表 → 详情（理解流程）→ **复制瘦身交接提示 / 在 Agent 打开** → 外部 Coding Agent  
- **次路径：** 从总览「有问题的应用」→ 问题详情 → 诊断（可选）→ 复制修复提示（默认不含诊断）→ Coding Agent  
- **S27a：** 交接文案在 `lib/handoff.js`（服务端）；不是业务解读/diagnose 的可配 LLM prompt；首期不提供设置页改全文模板
